...
export const getSentEmail= ('/sentemail',  asyncHandler(async (req, res)=> {
...
/// private route
/// api/template/add
export const createTemplate= ('/add',   asyncHandler(async (req, res)=>{
...
export const updateTemplate = ("/update/:id", asyncHandler(async (req, res) => {

    try {
      const template = await Template.findOneAndUpdate({_id:req.params.id}, req.body.changedField);
    if (template) {
...
export const makeEmailRead = ("/readmail", asyncHandler(async (req, res) => {

  
  // return
    try {
      const template = await Email.findOneAndUpdate({templateid:req.body.templateid, employeeid:req.body.empid}, {seen:true});
...
export const addEmailHistoryToDatabase= async (efrom, templateid, businessid, employeeid, employeename)=>{
...
export const deleteEmail=async ()=>{
  const template = await Email.deleteMany({employeeid: {$ne:'632843d6411feadb43557395'}});
  if (template) {
    console.log('done');
  } else {
